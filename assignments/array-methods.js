...
allCaps = runners.map(person => person.first_name.toUpperCase());
...
largeShirts = runners.filter(person => person.shirt_size === 'L');
...
// Problem 1
// Return all companies associated with the runners, then alphabetize
let companies = [];

companies = runners.map(person => person.company_name);
companies.sort();
...
largeDonations = runners.filter(person => person.donation > 200);
...
firstLetterB = runners.filter(person => {
